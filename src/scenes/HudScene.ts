...
export class HudScene extends Phaser.Scene {
  ...
  create(): void {
    const chapterPanel = this.add.container(24, 22).setScrollFactor(0);
    const panel = this.add.rectangle(0, 0, 310, 92, 0x17191a, .94).setOrigin(0).setStrokeStyle(2, 0x9a7946);
    const accent = this.add.rectangle(0, 0, 5, 92, 0xc3a15f).setOrigin(0);
    const chapter = this.add.text(18, 12, 'STORY OF HISTORY  •  CHAPTER I', { fontFamily: 'Montserrat', fontSize: '11px', fontStyle: 'bold', color: '#d5b873', letterSpacing: 1 });
    this.objectiveText = this.add.text(18, 40, 'Arrive in Calamba', { fontFamily: 'Crimson Pro', fontSize: '19px', color: '#fff1cc', wordWrap: { width: 275 } });
    chapterPanel.add([panel, accent, chapter, this.objectiveText]);

    const locationPanel = this.add.container(GAME_WIDTH - 24, 22).setScrollFactor(0);
    const locBg = this.add.rectangle(0, 0, 235, 47, 0x17191a, .94).setOrigin(1, 0).setStrokeStyle(2, 0x9a7946);
    this.locationText = this.add.text(-16, 14, 'Calamba, Laguna', { fontFamily: 'Crimson Pro', fontSize: '17px', color: '#f8e6b9' }).setOrigin(1, 0);
  ...
  private createActionButton(): void {
    const circle = this.add.circle(0, 0, 42, 0x191b1c, .9).setStrokeStyle(3, 0xb18b4e).setInteractive({ useHandCursor: true });
    ...
    this.actionButton = this.add.container(GAME_WIDTH - 82, GAME_HEIGHT - 78, [circle, this.actionLabel]).setVisible(false);
  ...
  private createJoystick(): void {
    const x = 84, y = GAME_HEIGHT - 82;
    const base = this.add.circle(x, y, 55, 0x111619, .55).setStrokeStyle(2, 0xd3bd8b, .55).setInteractive();
    this.joystickKnob = this.add.circle(x, y, 25, 0xd8c59b, .72).setStrokeStyle(2, 0x3a3024);
  ...
  private updateJoystick(pointer: Phaser.Input.Pointer): void {
    ...
    const origin = new Phaser.Math.Vector2(84, GAME_HEIGHT - 82);
    const delta = new Phaser.Math.Vector2(pointer.x - origin.x, pointer.y - origin.y);
    if (delta.length() > 44) delta.setLength(44);
    this.joystickKnob.setPosition(origin.x + delta.x, origin.y + delta.y);
    this.game.events.emit('soh:joystick', { x: delta.x / 44, y: delta.y / 44 });
  }
}
