...
export class HudScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const panel = this.add.rectangle(0, 0, 292, 62, 0x17191a, .92).setOrigin(0).setStrokeStyle(2, 0x9a7946);
    const portraitFrame = this.add.rectangle(9, 8, 46, 46, 0x25221d, 1).setOrigin(0).setStrokeStyle(1, 0xb18b4e);
    const portrait = this.add.image(32, 31, 'jose-idle-south-0').setScale(.55);
    const chapter = this.add.text(64, 10, 'STORY OF HISTORY  •  CHAPTER I', { fontFamily: 'Montserrat', fontSize: '9px', fontStyle: 'bold', color: '#d5b873', letterSpacing: 1 });
    const title = this.add.text(64, 30, 'Awaken in Calamba', { fontFamily: 'Crimson Pro', fontSize: '18px', fontStyle: 'bold', color: '#fff1cc' });
    const objectivePanel = this.add.rectangle(0, 69, 244, 36, 0x17191a, .92).setOrigin(0).setStrokeStyle(2, 0x9a7946);
    this.objectiveText = this.add.text(13, 78, 'Enter the Rizal family home', { fontFamily: 'Crimson Pro', fontSize: '14px', color: '#f4dfad', wordWrap: { width: 218 } });
    chapterPanel.add([panel, portraitFrame, portrait, chapter, title, objectivePanel, this.objectiveText]);
